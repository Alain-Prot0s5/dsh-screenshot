...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SnipButton() {
			...
			// 恢复输入框焦点：按钮点击 / 状态重渲染可能让输入框失焦，而监听器会在几秒后
			// （截图完成）盲发 Ctrl+V——焦点不在输入框就会"假粘贴"（日志记成功但图没进去）。
			const restoreFocus = () => {
				try {
					let el = prevFocus.current;
					if (!el || !document.contains(el) || typeof el.focus !== "function") {
						// 兜底：猜测输入框（富文本编辑器 / textarea）
						el = document.querySelector('[contenteditable="true"]') || document.querySelector("textarea");
					}
